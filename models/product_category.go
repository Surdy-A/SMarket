...
func (c *Category) CreateCategory(db *sql.DB) error {
	...
	err := db.QueryRow("INSERT INTO categories(id, main_category, sub_category) VALUES($1, $2, $3) RETURNING id",
...
func (c *Category) GetCategories(db *sql.DB) ([]Category, error) {
	rows, err := db.Query(`SELECT id, main_category, sub_category FROM categories`)
...
func (c *Category) GetCategory(db *sql.DB, id string) error {
	return db.QueryRow("SELECT * FROM categories WHERE id=$1", id).Scan(&c.ID, &c.MainCategory, &c.SubCategory)
}

func (c *Category) UpdateCategory(db *sql.DB) error {
	_, err := db.Exec(`UPDATE categories SET main_category=$1, sub_category=$2 WHERE id=$3`, c.MainCategory, c.SubCategory, c.ID)
